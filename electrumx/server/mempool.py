    ...
    async def _fetch_and_accept(self, hashes, all_hashes, touched):
        ...
        def deserialize_txs():    # This function is pure
            ...
            def create_atomical_from_definition(operation_found_at_inputs, tx, tx_hash, atomicals_updates_map):
                ...
                self.logger.info(f'Found Atomicals operation in mempool transaction {hash_to_hex_str(tx_hash)}, op={op}') 
                valid_create_op_type, mint_info = get_mint_info_op_factory(script_hashX, tx, tx_hash, operation_found_at_inputs)
                if valid_create_op_type:
                    atomical_id = mint_info['id']
                    self.logger.info(f'Valid Atomicals mint {valid_create_op_type} in mempool transaction {hash_to_hex_str(tx_hash)}, atomicalId={location_id_bytes_to_compact(atomical_id)}') 
