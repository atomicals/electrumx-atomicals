...
class DB:
    ...
    async def _open_dbs(self, for_sync: bool, compacting: bool):
        ...
        # Read Realm number counts (requires meta directory)
    ...
    def flush_dbs(self, flush_data, flush_utxos, estimate_txs_remaining):
        ...
        self.logger.info(f'flush #{self.history.flush_count:,d} took '
                         ...
                         f'Realm txs: {flush_data.realm_count:,d} ({realm_delta:+,d})')
    ...
    def backup_fs(self, height, tx_count, atomical_count, realm_count):
    ...
    def get_realm_mint_info_dump(self, realm_id):
        mint_info_value_dump = self.utxo_db.get(b'ri' + realm_id)
        if not mint_info_value_dump:
            raise IndexError(f'get_realm_mint_info_dump {realm_id.hex()} mint db record not found. Index error.')
        return mint_info_value_dump
    ...
    async def get_prefix_nodes(self, realm_id, prefix):
        def read_prefix_branch_path():
            prefix = b'r' + realm_id + prefix
            prefix_found = False
            for db_key, db_value in self.utxo_db.iterator(prefix=prefix):
                if len(db_value) % 36 != 0:
                    raise IndexError(f'get_prefix_nodes db_value not multiple of 36 {realm_id.hex()} {prefix.hex()}')
                return {
                    'realm_id': realm_id.hex(),
                    'prefix_requested': prefix.hex(),
                    'prefix_match': db_key.hex(),
                    'outpoints': db_value.hex()
                }
            return {}
        return await run_in_thread(read_prefix_branch_path)
